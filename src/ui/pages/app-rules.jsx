...
import { Box, Typography, List, ListItem, ListItemText, Chip, Tooltip, IconButton, Button } from '@mui/material';
...
const RuleActionLabel = ({action}) => {
    return (
        <Typography variant="button" fontSize={11}>
            {action === "disable" ? "Disabled" : action === 'exclusive' ? 'Exclusive' : 'Enabled'}
        </Typography>
    );
};
        ...
        secondaryAction={
            <Box sx={{
                ...
                <Chip 
                    ...
                    color={rule.action === "disable" ? "error" : rule.action === 'exclusive' ? 'warning' : 'success'}
                />
    ...
    return (
        <Box sx={{
            ...
            <Typography variant="body2" color="GrayText" sx={{ mb: 1, mt: 1 }}>
                These rules allow you to control the behavior of the sound daemon based on the currently running applications.
            </Typography>
            <List sx={{
                overflow: 'auto',
